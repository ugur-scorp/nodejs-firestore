...
if staging.is_dir():
    ...
    # Copy the admin library from staging.
    for version in ["v1"]:
        library = staging / "admin" / version
        _tracked_paths.add(library)
        s.copy(library, "dev", excludes=["package.json", "README.md", "src/index.ts", "src/v1/index.ts",
            "tsconfig.json", "linkinator.config.json", "webpack.config.js"])

    # Copy the firestore libraries from staging.
    for version in ["v1beta1", "v1"]:
        library = staging / version
        _tracked_paths.add(library)
        s.copy(library, "dev", excludes=["package.json", "README.md", "src/index.ts", f"src/${version}/index.ts",
            "tsconfig.json", "linkinator.config.json", "webpack.config.js"])
